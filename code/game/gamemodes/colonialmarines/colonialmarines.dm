...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_USASF_POINT_LOMA)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "Children. You have broken free of the Hunter's containment and ravaged this ground.\n\nYet... I sense that your fight for freedom is not yet over.\n\nSecure this ground and withstand any further attacks from the tallhosts.", "everything", QUEEN_MOTHER_ANNOUNCE), 30 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Planetary Scan Concluded:\n\n- Scan reveals dropship debris scattered across aerospace base surface.\n- Several sinkholes have opened due to geological instability.\n- Power failing across the colony.", "ARES V3.2", 'sound/AI/commandreport.ogg'), 2 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(elder_overseer_message), "A Hunting Party has lost control of their ship, and it has crashed inside of a Human Colony. The serpents inside have been released.", 1 MINUTES))
